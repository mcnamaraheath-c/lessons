...
Machinery: same EN/ES toggle, vocab tooltips, progress dots (one per case), single offline file. **Read-aloud (Aug 2026):** a header "Read aloud" button speaks the current screen via the browser speech API — no cloud audio, keeping the file small and offline. `pickVoice()` ranks the device's voices (Natural/Neural > Google > others, eSpeak penalized) and matches the EN/ES toggle; text is collected from the rendered screen with tooltips/buttons/textareas stripped, table rows read cell-by-cell with pauses, and chunked into sentences (a Chrome long-utterance workaround — keep the chunking). Speech stops on navigation or language switch. The button hides itself on browsers without speech support. **Lightbox (Aug 2026):** clicking (or Enter/Space on) any `figure.exhibit img` opens it full-screen with its caption; closes on overlay click, the × button, Escape, or navigation. Wired by event delegation on `#stage` plus a `tagEnlargeableImages()` pass in `render()` — new images get it automatically as long as they use the `figure.exhibit` structure. State is `{language, screen, verdicts}`; content lives in the `CASES` array. Nothing can be permanently failed; verdicts can be revisited via Back.
